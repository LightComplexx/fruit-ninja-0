//
// Grocer.cpp
//

// System includes.
#include <stdlib.h>    // for rand()

// Engine includes.
#include "EventStep.h"
#include "GameManager.h"
#include "LogManager.h"
#include "WorldManager.h"

// Game includes.
#include "Fruit.h"
#include "GameOver.h"
#include "Grocer.h"
#include "EventBonus.h"
#include "Ninja.h"
#include "Sword.h"
#include "InputHandler.h"

Grocer::Grocer(bool isBonusStage) {
	setType(GROCER_STRING);
	setSolidness(df::SPECTRAL);
	setVisible(false);
	registerInterest(df::STEP_EVENT);
	registerInterest(BONUS_EVENT);

	is_bonus_stage = isBonusStage;
	m_wave = 1;

	if (is_bonus_stage) {
		m_wave_end = B_WAVE_LEN;
		m_wave_speed = B_WAVE_SPEED; // Starting speed (spaces/tick).
		m_wave_spawn = B_WAVE_SPAWN; // Starting spawn rate (ticks).
	}
	else {
		m_wave_end = WAVE_LEN;
		m_wave_speed = WAVE_SPEED; // Starting speed (spaces/tick).
		m_wave_spawn = WAVE_SPAWN; // Starting spawn rate (ticks).
	}

	m_spawn = m_wave_spawn;
}

// Handle event.
// Return 0 if ignored, else 1.
int Grocer::eventHandler(const df::Event* p_e) {

	// Step event.
	if (p_e->getType() == df::STEP_EVENT)
		return step((df::EventStep*)p_e);

	// If get here, have ignored this event.
	return 0;
}

// Handle step event.
int Grocer::step(const df::EventStep* p_e) {

	// Fruit grocer.
	m_spawn -= 1;
	if (m_spawn < 0) {

		int mod = m_wave + 1 > NUM_FRUITS ? NUM_FRUITS : m_wave + 1;
		Fruit* p_f = new Fruit(FRUIT[rand() % mod], is_bonus_stage);
		if (!p_f) {
			LM.writeLog("Grocer::step(): Error! Unable to allocate Fruit.");
			return 0;
		}

		p_f->start(m_wave_speed);

		m_spawn = m_wave_spawn;
	}

	// Advance wave.
	m_wave_end -= 1;
	if (m_wave_end < 0) {
		if (is_bonus_stage) {
			m_wave_end = B_WAVE_LEN;
			m_wave_spawn += B_SPAWN_INC; // Increase spawn rate.
			m_wave_speed += B_SPEED_INC; // Increase Fruit speed.
		}
		else {
			m_wave_end = WAVE_LEN;
			m_wave_spawn += SPAWN_INC; // Increase spawn rate.
			m_wave_speed += SPEED_INC; // Increase Fruit speed.
		}
		m_wave += 1;
		if (m_wave == NUM_WAVES + 1)
			this->gameOver();
	}

	return 1;
}

// Do game over actions.
void Grocer::gameOver() {
	// Create G-A-M-E O-V-E-R object.
	new GameOver(is_bonus_stage);

	// Destroy all remaining Fruit (no points).
	df::ObjectList ol_f = WM.solidObjects();
	for (int i = 0; i < ol_f.getCount(); i++)
		if (dynamic_cast <Fruit*>(ol_f[i]))
			WM.markForDelete(ol_f[i]);

	// Destroy the sword
	deleteObjectsOfType("Sword");

	// If in bonus stage, find and destroy InputHandler
	if (is_bonus_stage)
		deleteObjectsOfType("InputHandler");

	WM.markForDelete(this);
}

void Grocer::deleteObjectsOfType(const std::string& type) {
	df::ObjectList ol = WM.objectsOfType(type);
	for (int i = 0; i < ol.getCount(); i++)
		WM.markForDelete(ol[i]);
}
