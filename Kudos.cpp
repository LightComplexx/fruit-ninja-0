...
Kudos::Kudos(bool isBonusStage) {
	setType(KUDOS_STRING);
	setSolidness(df::SPECTRAL);
	setAltitude(df::MAX_ALTITUDE);
	setSprite("kudos");

	// Set bonus stage flag
	...
	if (p_sprite) {
		// If bonus stage, only spawn electric kudos
		int kudos;
		if (is_bonus_stage) 
			kudos = p_sprite->getFrameCount() - 1;
		else {
			int count = p_sprite->getFrameCount() - 1;
			kudos = rand() % count;
		}
	...
	// If bonus stage, spawn above ninja
	// Else, Pick random location.
	df::Vector p;
	if (is_bonus_stage) {
		p = df::Vector(WM.getBoundary().getHorizontal() / 4 + 10,
			WM.getBoundary().getVertical() / 4 - 1);
	}
	else {
		p = df::Vector(WM.getBoundary().getHorizontal() / 8 +
